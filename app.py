...
async def upload_pdf(file: UploadFile = File(...)):
    print(f"DEBUG: Recebido arquivo: {file.filename}")
    try:
        contents = await file.read()
        print(f"DEBUG: Tamanho do arquivo: {len(contents)} bytes")
        
        # Read the PDF from binary stream
        doc = pymupdf.open(stream=contents, filetype="pdf")
        print(f"DEBUG: PDF aberto com sucesso. Total de páginas: {len(doc)}")
        ...
        for page in doc:
            ...
            pagina += 1
        print(f"DEBUG: Extração concluída. Itens encontrados: {len(extracted_data)}")
        return {"data": extracted_data}
    except Exception as e:
        print(f"ERROR: Falha na extração: {str(e)}")
